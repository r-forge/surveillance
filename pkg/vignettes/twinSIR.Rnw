...
%\VignetteDepends{surveillance, quadprog, timereg, survival}
...
knitr::opts_chunk$set(echo = TRUE, tidy = FALSE, results = 'markup',
                      fig.path='plots/twinSIR-', fig.width = 8, fig.height = 4,
...
\Shorttitle{Modeling the SIR event history of a fixed population}
...
``\code{twinSIR}'' of \citet{hoehle2009} include the spread of livestock
diseases across farms, household models, and epidemics across networks.
...
isolated German village Hagelloch in the year 1861.
...
The spatio-temporal point process model ``\code{twinstim}''
(\citealp{meyer.etal2011}, see also \code{vignette("twinstim")})
...
the conditional intensity function of the underlying point process
...
endemic component consists of a Cox proportional hazards formulation
...
model~\citep{martinussen_scheike2002}. As a consequence, the
\code{twinSIR} model could in principle be fitted with the \CRANpkg{timereg}
package~\citep{R:timereg}, which yields estimates for the cumulative
...
the \class{Surv} class of package \CRANpkg{survival}~\citep{R:survival} and also
used in \CRANpkg{timereg} \citep{R:timereg}.
Specifically, the observation period is splitted up into consecutive time
...
and a \code{stateplot} shows the changes for a selected unit.
...
<<hagellochFit_profile_plot, fig.cap="Normalized log-likelihood for $\\alpha_{c1}$ and $\\alpha_{c2}$ when fitting the \\code{twinSIR} model formulated in Equation~\\eqref{eqn:twinSIR:hagelloch} to the Hagelloch data.", fig.pos="ht">>=
plot(prof)
@
...
For example, to investigate how the CIF decomposes into endemic and epidemic
intensity over time, we produce Figure~\ref{fig:hagellochFit_plot1} by:
<<hagellochFit_plot, echo=2, fig.width=5, fig.height=3.5, out.width="0.47\\linewidth", fig.subcap=c("Epidemic proportion.","Transformed residuals."), fig.cap="Diagnostic plots for the \\code{twinSIR} model formulated in Equation~\\ref{eqn:twinSIR:hagelloch}.">>=
par(mar = c(5, 5, 1, 1))
...
Finally, \class{twinSIR}'s \code{AIC}-method computes the one-sided
AIC~\citep{hughes.king2003} as described in \citet{hoehle2009}, which can be
used for model selection
under positivity constraints on $\bm{\alpha}$.
For instance, we may consider a more flexible model for local spread using a
...
A random seed was set since the parameter penalty in the one-sided AIC is
determined by Monte Carlo simulation. The algorithm is described in
...
## "quadprog" and "timereg" need manual author formatting
Rbib1 <- sapply(c("quadprog", "timereg"), function (pkg) {
    bib <- citation(pkg, auto = TRUE)
    bib$key <- paste("R", pkg, sep=":")
    bib
}, simplify=FALSE, USE.NAMES=TRUE)
Rbib1$quadprog$author <- "Berwin A. Turlach"
Rbib1$quadprog$note <- paste0(Rbib1$quadprog$note,
    ", ported to \\proglang{R} by Andreas Weingessel")
Rbib1$timereg$author <- "Thomas Scheike"
## write to bibfile
