...
\title{Outbreak detection with the \textsf{R} package \surveillance}
\author{
...
\SweaveOpts{prefix.string=plots/surveillance}
...
to the package: this includes various modelling frameworks
...
These new features are described in \citet{meyer.etal2014} and
\citet{salmon.etal2014}, respectively, and
corresponding vignettes are included in the package;
