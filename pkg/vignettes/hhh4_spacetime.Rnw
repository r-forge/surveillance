...
@
We visualize the \code{exp}-transformed values (multiplicative effects)
on component-specific maps using the plot \code{type = "ri"}
...
for (comp in c("ar", "ne", "end")) {
  print(plot(measlesFit_ri, type = "ri", component = comp, log = FALSE,
    col.regions = cm.colors(100), labels = list(cex = 0.6)))
}
@
