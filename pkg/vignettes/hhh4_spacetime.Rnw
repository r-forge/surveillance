...
derive the matrix of adjacency orders automatically\footnote{
  Visual inspection of the result is recommended, e.g., via labelling the
  districts in the map with the number of identified first-order neighbors
  (the crucial \code{poly2adjmat} step).
} using the functions
...
weserems_nbOrder <- nbOrder(poly2adjmat(map), maxlag = Inf)
@
