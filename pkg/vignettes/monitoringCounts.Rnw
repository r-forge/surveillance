...
<<boda, echo = F, include = F, message = F>>=
if (computeALL) {
library("INLA")
...
@
<<boda2, echo = F, include = F, message = F>>=
if (computeALL) {
<<NICELOOKINGboda2>>
...
Figure~\ref{fig:alarmplot}. Note 2018-07-19: We currently have to use the argument `prior = "iid"` in both calls of the `boda` function, because the procedure crashes when using the newest version of the INLA software crashes with argument `prior = "rw1"`. This means results in this vignette deviate from the results reported in the JSS paper - in particular we do not get any alarms when using the boda procedure with covariates. We are looking into the problem.
...
@
<<cath, echo = F, include = F, message = F>>=
