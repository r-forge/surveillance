%\VignetteIndexEntry{hhh4: Endemic-epidemic modelling of infectious disease counts}
...
% Sweave 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\SweaveOpts{prefix.string=plots/hhh4, width=10,height=4.5, keep.source=T, strip.white=true}
...
\begin{abstract}
  ...
  disease counts. For the special case of \emph{spatio-temporal} \hhh\ models,
  i.e., areal time series of counts, see the extra
  \code{vignette("hhh4\_spacetime")} \citep[based on][Section~5]{meyer.etal2014}.
  The function \hhh\ is part of the \R\ package \surveillance, 
  which provides tools for the visualization, modelling and monitoring of 
  epidemic phenomena.
  An overview of the outbreak detection functionality of \surveillance\ is
  given by \citep{salmon.etal2014}, and -- slightly outdated -- in
  \code{vignette("surveillance")} and in the original paper of \citet{hoehle-2007}.
...
\begin{center}
\vspace*{-2em}
\includegraphics[width=0.5\textwidth]{plots/hhh4-nhoodByBw}
\end{center}
