...
For instance, a disease map of the year 2001 can be obtained as follows:
\setkeys{Gin}{width=.5\textwidth}
\begin{center}
<<plot-flu-ByBw, fig=TRUE, width=4.5, height=4>>=
data("fluBYBW")
...
have been analyzed by \citet{herzog-etal-2010} after aggregation into successive
bi-weekly periods.
...
spatial context using a power-law formulation based on the order of adjacency.
...
\citep[cf.][Table~3, model~B2]{paul-held-2011}.
