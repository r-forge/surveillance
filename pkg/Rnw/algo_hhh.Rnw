...
\arguments{
        ...
        \item{control}{Control object:
              ...
              \item{\code{linear}}{a \code{logical} (or a vector of logicals) indicating wether a linear 
                    ...
                    is included}
              \item{\code{season}}{\code{list} with integer number of Fourier frequencies \code{nseason} 
                    and period length \code{period} as elements. If \code{nseason} is a vector
...
model1 <- list(lambda=TRUE, linear=TRUE, 
               season=list(nseason=1), negbin="single")
...
model2 <- list(lambda=TRUE, neighbours=TRUE, 
               linear=FALSE, season=list(nseason=1), 
               negbin="single")
...
\arguments{
        ...
        \item{control}{Control object:
              \itemize{
              \item{lambda}{if \code{TRUE} an autoregressive 
                   parameter \eqn{\lambda} is included, if \code{lambda} is a vector of logicals,
                   unit-specific parameters \eqn{\lambda_i} are included}
              \item{neighbours}{if \code{TRUE} an autoregressive parameter for 
                    adjacent units \eqn{\phi} is included, if \code{neighbours} is a vector of logicals,
                    unit-specific parameters \eqn{\phi_i} are included}
              \item{linear}{a \code{logical} (or a vector of logicals) indicating wether a linear 
                    trend \eqn{\beta} (or a linear trend \eqn{\beta_i} for each unit)
                    is included}
              \item{season}{\code{list} with integer number of Fourier frequencies \code{nseason} 
                    and period length \code{period} as elements. If \code{nseason} is a vector
                    of integers, each unit \eqn{i} gets its own seasonal parameters }
              \item{negbin}{if \code{"single"} negative binomial rather than poisson is used,
                    if \code{"multiple"} unit-specific overdispersion parameters are used.}
              \item{proportion}{see details in \code{\link{meanResponse}} }
...
model2 <- list(lambda=TRUE, neighbours=TRUE, negbin="single",
               season=list(nseason=1, period=12))
...
model <- list(lambda=c(TRUE,TRUE), neighbours=c(FALSE,TRUE),
              linear=FALSE,season=list(nseason=c(3,1)),negbin="multiple")
...
\arguments{
   ...
   \item{control}{specified model}
	\item{params}{list of parameters: \code{param=c(lower,upper,length)}
		\itemize{
		\item{epidemic}{autoregressive parameters \eqn{\lambda} and \eqn{\phi}. }
		\item{endemic}{trend and seasonal parameters \eqn{\beta,\gamma_j}. }
		\item{negbin}{overdispersion parameter for negative binomial model \eqn{\psi}. }
		} }
}
...
cntrl1 <- list(lambda=TRUE, neighbours=TRUE, 
               linear=TRUE, season=list(nseason=1))
