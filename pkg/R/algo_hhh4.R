...
setControl <- function(control, stsObj){
  ...
  if(is.null( control[["end", exact = TRUE]] )) {
    ...
    } else if(!is.matrix(control$end$offset)){
      stop("\'end$offset\' must be a matrix of size ",nTime,"x",nUnits,"\n")        
    } else if((ncol(control$end$offset) != nUnits) | (nrow(control$end$offset) != nTime)){
      stop("\'end$offset\' must be a matrix of size ",nTime,"x",nUnits,"\n")        
...
print.ah4 <- function(x,digits = max(3, getOption("digits") - 3),reparamPsi=TRUE,...){
  if(!x$convergence)
    ...
    if(x$dim["random"]==0){
      ...
      cat('\npenalized log-likelihood: ',round(x$loglik,digits=digits-2),'\n')  
...
logLik.ah4 <- function(object,...){
   if(!inherits(object, "ah4"))
      stop("expected object to be an object of class ah4\n")
...
fixef <- function(x){
  if(x$dim[1]>0){
    return(head(x$coefficients, x$dim[1]))
  } else return(NULL)
}

ranef <- function(x){
  if(x$dim[2]>0){
    return(tail(x$coefficients, x$dim[2]))
  } else return(NULL)
}
