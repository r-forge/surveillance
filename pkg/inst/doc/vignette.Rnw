...
\author{
Michael Höhle\thanks{Author of correspondance: Department of Statistics, University of Munich, Ludwigstr.\ 33, 80539 München, Germany, Email: \texttt{hoehle@stat.uni-muenchen.de}}$\>\>$, Andrea Riebler and Michaela Paul\\
...
control <- lapply(control,function(cntrl) { cntrl$range=range;return(cntrl)})
...
For example, consider the weekly counts of new measels cases for each
...
In the package \texttt{surveillance} the measels data are already available
in the form of a \texttt{disProg}-object.

<<fig=T,width=13,height=6>>=
data(measels.weser)
plot(measels.weser, title="Measels in Weser-Ems 2001-2002", xaxis.years=TRUE, startyear= 2001, firstweek=1)
@
...
y-axis is the number of measels cases.

<<fig=T,width=13,height=9>>=
plot(measels.weser,as.one=FALSE,xaxis.years=FALSE)
@
\vspace{1em}
...
if(compute){
  # algo.hhh
  cntrl <- list(linear=TRUE, nseason=1, period=52, neighbours=FALSE, negbin=TRUE, lambda=TRUE)
  measels.hhh.neF <- algo.hhh(measels.weser,control=cntrl)

  cntrl$neighbours <- TRUE
  measels.hhh <-algo.hhh(measels.weser,cntrl)

  # algo.hhh.grid
  cntrl <- list(linear=TRUE, nseason=1, period=52, neighbours=TRUE, negbin=TRUE, lambda=TRUE)
  grid <- create.grid(params=list(beta=c(lower=-0.5,upper=0.5,length=3), gammaDelta=c(-0.5,0.5,3), nseason=1, phi=c(0.1,0.9,5),  psi=c(0.3,12,5), lambda=c(0.1,0.9,5)))
  cat("running a grid search for up to 900 seconds.\n")
  measels.hhh.grid <- algo.hhh.grid(measels.weser, control=cntrl, thetastartMatrix=grid, maxTime=10)#,verbose=T)
}
@

For the weekly measels data $\omega_s=2s\pi/52$ (i.e.\ \texttt{period}=52).
...
<<echo=T,eval=F>>=
cntrl <- list(linear=TRUE, nseason=1, period=52, neighbours=TRUE, negbin=TRUE, lambda=TRUE)
measels.hhh <- algo.hhh(measels.weser,control=cntrl)
@
...
<<echo=T,eval=F>>=
cntrl <- list(linear=TRUE, nseason=1, period=52, neighbours=TRUE, negbin=TRUE, lambda=TRUE)
grid <- create.grid(list(beta=c(-0.5,0.5,3), nseason=1, gammaDelta=c(-0.5, 0.5, 3), phi=c(0.1, 0.9, 5),  psi=c(0.3, 12, 5), lambda=c(0.1, 0.9, 5)))
algo.hhh.grid(measels.weser, control=cntrl, thetastartMatrix=grid, maxTime=1800)#,verbose=T)
@
<<echo=F>>=
print(measels.hhh.grid,3)
@

<<echo=F>>=
#save computed results
save(list=c("sts.cdc","sts.farrington","acall","res","res.survstat","control","ten.surv",ls(pattern="measels")),file=".Rresults")
@
