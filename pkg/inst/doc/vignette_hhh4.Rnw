...
######################################################################
compute <- !file.exists("hhh4results.RData")
#load computed results
if(!compute) load("hhh4results.RData")
...
flu.counts <- as.matrix(read.table(system.file("data/flu_ByBw.txt",package="surveillance")))
# read in adjacency matrix with elements 1 if two regions share a common border
nhood <- as.matrix(read.table(system.file("data/neighourhood_ByBw.txt",package="surveillance")))
...
p <- as.matrix(read.table(system.file("data/population_2001-12-31_ByBw.txt",package="surveillance")))
# create sts object
