...
% Sweave 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\SweaveOpts{prefix.string=figs/vignette_hhh4, width=10,height=4.5, keep.source=T, strip.white=true}
...
######################################################################
compute <- !file.exists(".hhh4results")
#load computed results
if(!compute) load(".hhh4results")
...
They can be easily converted into the easier to handle S4 \sts\ data structure using
the function \code{disProg2sts}. The resulting \sts\ object can accessed 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Finally, the spatio-temporal spread of influenza in the 140 Kreise (districts) 
of Bavaria and Baden-W\"urttemberg  is analyzed using the weekly number of 
cases reported to the RKI~\citep{survstat-fluByBw}.
...
@
<<flu-ByBw, fig=T,width=6,height=6, include=F,echo=F>>=
par(mar=c(0,0,0,0))
plot(flu[year(flu) == 2001, ],    # select year 2001
     type = observed ~ 1 | unit,  # map of counts aggregated over times t
     labels = FALSE               # suppress region labels in map
...
@
\begin{center}
\vspace*{-2em}\includegraphics[width=.4\textwidth]{figs/vignette_hhh4-flu-ByBw}
\end{center}
...
@
<<measles, fig=T, echo=F>>=
load("../data/measles_laender_2005-2007.RData")
measles2w <- aggregate(measles.land, nfreq = 26)
...
mean
\begin{align*}
  \mu_{it} = \lambda_{it} y_{i,t-1} + \phi_{it} \sum_{j} w_{ji} y_{j,t-1} +e_{it} \nu_{it}
\end{align*}
...
represented as $\bm{z}_{it}=(t,\sin(2\pi/\code{freq}),\cos(2\pi/\code{freq}))^\top$.
...
plot(result, i = 2, col = c("orange", "blue", "grey90"), legend = TRUE)
@
\end{center}
...
high-dimensional multivariate time series of counts) the use of region-specific 
...
if(compute){   
  ...
  save(result.B2, pred.B2, file=".hhh4results")
}
@
...
vac0 <- 1-adjustVac(vac2006[1:16,3:4],p=0.5,nrow=measles2w@freq*3)
...
The $\Sexpr{nrow(vac0)}\times \Sexpr{ncol(vac0)}$ matrix \code{vac0} contains 
...
of infectious disease counts. The discussed count data model provides 
a flexible framework, where serial and spatio-temporal, as well as
heterogeneity in incidence levels and disease transmission can be accounted
for. The functionality of \hhh\ was illustrated using several built-in data 
sets.
