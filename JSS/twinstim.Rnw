...
\section[Individual case reports]{\code{twinstim}: individual case reports} \label{sec:twinstim}
  ...
  \begin{itemize}
    \item Make a table listing all functions for twinstim intended to be used
...
\subsection{The reproduction number} \label{twinstim:R0}
...
infection.
The reproduction number $\mu_j$ can also be motivated by looking at
a spatio-temporal version of the simple SIR model \eqref{kermack.mckendrick}
established from the point process model \eqref{eqn:twinstim}:
...
\eqref{eqn:R0:kermack.mckendrick} known from the simple SIR model, by
interpreting $\abs{\bm{W}}$ as the population size, $\beta$ as the transmission
rate and $\alpha$ as the removal rate.
Like in classic epidemic models, the process is sub-critical if $\mu < 1$ holds,
which means that its eventual extinction is almost sure. However, it is crucial
to understand that in the full point process model \eqref{eqn:twinstim} with an
endemic component, new infections may always occur via ``immigration''.
Hence, the reproduction number as estimated from this model is adjusted for such
new infections occurring independently of previous infections. It therefore
tends to be lower than the one estimated from a classical non-spatial SIR model
without immigration of infectives.
